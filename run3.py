...
df = pd.read_csv('./data/NYQ.csv')
df = df[(df.Type == 'S') & (df.TypeDisplay == 'Equity')].sort_values('Type')
all_tickers = df.loc[:, 'Ticker']
...
j = 0
for i in chunker(df, 500):
    all_tickers = i.loc[:, 'Ticker']
    ...
    try:
        ...
        print('fuck with ' + str(all_tickers.tolist()))
        continue
    with open('./data/downloads/NYQ/1day/chunk_' + str(j) + '.pkl', 'wb') as output:  # Overwrites any existing file.
